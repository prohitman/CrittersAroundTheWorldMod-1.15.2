...
public ModRenderTypes(String nameIn, VertexFormat formatIn, int drawModeIn, int bufferSizeIn, boolean useDelegateIn,
	...
	public static RenderType getBumLayer(ResourceLocation locationIn) {
	      ...
	      return makeType("eyes", DefaultVertexFormats.ENTITY, 7, 256, false, true, RenderType.State.getBuilder().texture(renderstate$texturestate).transparency(ADDITIVE_TRANSPARENCY).alpha(HALF_ALPHA).writeMask(COLOR_WRITE).fog(BLACK_FOG).build(false));
	   }
}
